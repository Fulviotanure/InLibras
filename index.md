...
<head>
    ...
    <script type="module">
        ...
        onAuthStateChanged(auth, (user) => {
            ...
            if (!user) {
                ...
                // Se estiver logado, atualiza as informações do usuário
            ...
            */
            ...
            document.getElementById('userName').textContent = 'Usuário Teste';
            document.getElementById('userEmail').textContent = 'teste@inlibras.com.br';
            document.getElementById('lastLogin').textContent = new Date().toLocaleString();
        });
        ...
        window.logout = async function() {
            try {
                ...
                // Simulação de logout
                alert('Logout desativado temporariamente para testes');
            } catch (error) {
    ...
    <style>
        ...
        .header::before {
            ...
            right: 0;
            bottom: 0;
            background: url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><path d="M0,0 L100,100 M100,0 L0,100" stroke="rgba(255,255,255,0.1)" stroke-width="2"/></svg>');
            opacity: 0.1;
        }
        .logo-container {
            width: 120px;
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <header class="header p-8">
            <div class="flex items-center justify-center">
                <img src="../assets/images/InLibras.png" alt="InLibras" class="h-64">
            </div>
            <p class="app-subtitle">Calculadora de Orçamento de Interpretação de Libras</p>
        </header>

        <!-- User Container -->
        <div class="glass-effect p-4 rounded-xl mb-6 relative">
            <div class="flex items-center justify-end">
                <div class="relative group">
                    <button onclick="toggleUserMenu()" class="flex items-center space-x-2 bg-white px-4 py-2 rounded-lg shadow-md hover:bg-gray-50 transition duration-200" aria-label="Menu do usuário">
                    ...
                    <div id="userMenu" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg py-2 z-10">
                        ...
                        <div class="px-4 py-2 border-b border-gray-100">
                            ...
                            <p id="lastLogin" class="text-sm font-medium text-gray-700"></p>
                        </div>
                        <button onclick="logout()" class="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 transition duration-200">
                    Sair
                </button>
            </div>
                </div>
        ...
        <div class="card-modern">
            <h2 class="text-2xl font-bold text-gray-800 mb-6">Orçamento de Interpretação</h2>
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="valorHoraInterprete" class="block text-sm font-medium text-gray-700 mb-2">
                            Valor Hora por Intérprete (R$)
                        </label>
                        <input type="number" id="valorHoraInterprete" placeholder="0.00" min="0" step="0.01"
                               class="input-modern w-full">
                    </div>

                    <div>
                        <label for="quantidadeInterpretes" class="block text-sm font-medium text-gray-700 mb-2">
                            Quantidade de Intérpretes
                        </label>
                        <input type="number" id="quantidadeInterpretes" placeholder="0" min="0" step="1"
                               class="input-modern w-full">
                    </div>
                </div>
